...
app.get("/urls", middlewares.auth, (req, res) => {
  res.render("urls_index", {urls: service.fetchURLByID(req.cookies.user_id.id),  user: req.cookies.user_id, error: null});
});

app.post("/urls", middlewares.auth, (req, res) => {
  let newURL = service.createNewURL(req.body.longURL, req.cookies.user_id.id)
...
app.get("/urls/new", middlewares.auth, (req, res) => {
  res.render("urls_new",  { user: req.cookies.user_id, error: null} );
});
app.post("/urls/:shortURL", middlewares.auth, (req, res) => {
  service.createNewURL(req.body.longURL, req.cookies.user_id.id)
  res.redirect('/urls')
});

/***URL EDIT***/
app.get("/urls/:shortURL", middlewares.auth, (req, res, next) => {
  let url = service.getURLRestricted(req.params.shortURL, req.cookies.user_id.id);
  if (url === null) {
    next();
  } else {
    res.render("urls_show", {url, user: req.cookies.user_id, error: null});
  }
});
app.post("/url/:shortURL", middlewares.auth, (req, res) => {
  ...
  service.editURL(req.params.shortURL, req.body.longURL, req.cookies.user_id.id);
  res.redirect('/urls')
});

/***DELETE URL***/
app.post('/url/:shortURL/delete', middlewares.auth, (req, res) => {
  service.deleteURL(req.params.shortURL, req.cookies.user_id.id)
  res.redirect('/urls');
})
